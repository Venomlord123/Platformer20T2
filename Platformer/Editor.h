...
public:
	...
	//check for click, if so, pass a function as an arguemnt to store
...
{
	//in the last weeks editor script
private:
	virtual void draw(sf::RenderTarget& target, sf::RenderStates states) const
	{
		target.draw(rect, states);
...
public:
	...
	//keep track if we're painting with an actor, or a tile
	...
	void save(std::string fileName, Tile** incTile)
	{
		...
		if (myfile.is_open())
		{
			...
			//loop through all of the lists, add them to the save file, then add player/exit positions
			//coins
			if (!coinPos.empty())
			{
			...
			while (std::getline(myfile, line))
			{
				...
				else if (b >= x) //loading actors
				{
					...
					if (line[0] == 'C' && line[1] == 'o') //[C][o]ins
					{
						...
						while (curEnd < lineHold.length() && lineHold.find('(') != std::string::npos) //if the ')' ins't the last in the line
						{
					...
					else if (line[0] == 'E' && line[1] == 'n')
					{
						...
						while (curEnd < lineHold.length() && lineHold.find('(') != std::string::npos) //if the ')' ins't the last in the line
						{
							...
							incTile[std::stoi(xStr)][std::stoi(yStr)].actor.ChangeActor(Actor::Type::Enemy);
							lineHold[curStart] = '<';
							lineHold[curEnd] = '>';
						}
					}
					else if (line[0] == 'S' && line[1] == 'p')
					{
						...
						while (curEnd < lineHold.length() && lineHold.find('(') != std::string::npos) //if the ')' ins't the last in the line
						{
							...
							incTile[std::stoi(xStr)][std::stoi(yStr)].actor.ChangeActor(Actor::Type::Spike);
							lineHold[curStart] = '<';
							lineHold[curEnd] = '>';
						}
					}
					else if (line[0] == 'P' && line[1] == 'l')
					{
						...
						while (curEnd < lineHold.length() && lineHold.find('(') != std::string::npos) //if the ')' ins't the last in the line
						{
							...
							incTile[std::stoi(xStr)][std::stoi(yStr)].actor.ChangeActor(Actor::Type::Player);
							lineHold[curStart] = '<';
							lineHold[curEnd] = '>';
						}
					}
					else if (line[0] == 'E' && line[1] == 'x')
					{
						...
						while (curEnd < lineHold.length() && lineHold.find('(') != std::string::npos) //if the ')' ins't the last in the line
						{
							...
							incTile[std::stoi(xStr)][std::stoi(yStr)].actor.ChangeActor(Actor::Type::Exit);
							lineHold[curStart] = '<';
							lineHold[curEnd] = '>';
						}
					}
